# SEC differential analysis — E. coli complexome, metabolite treatments (PMC)

Reproducible RStudio project for the **size-exclusion chromatography (SEC)
differential analysis** of native *E. coli* lysate treated with individual
metabolites (ATP, ADP, NAD, aKG, PEP, PGP), each compared against an untreated
control, built on
...
6. **Knit** `analysis/DiffAnalysis_Ecoli_PMC.Rmd` (the *Knit* button), or run
   it chunk by chunk.
...
│   ├── DiffAnalysis_Ecoli_PMC.Rmd   # ← MAIN, cleaned pipeline  (being built — see HANDOVER_NOTES.md)
